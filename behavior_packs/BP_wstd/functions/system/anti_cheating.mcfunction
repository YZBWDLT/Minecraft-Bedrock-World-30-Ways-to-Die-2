...
# ===== 防止利用特性卡bug =====
# 在非28关时清除船
execute @e[name=level,scores={backend=!28}] ~~~ execute @e[type=boat] ~~~ tellraw @a {"rawtext":[{"translate":"chat.boat_not_allowed"}]}
execute @e[name=level,scores={backend=!28}] ~~~ execute @e[type=boat] ~~~ kill @s

# 在非18关时清除末影珍珠
execute @e[name=level,scores={backend=!31,backend=!18}] ~~~ execute @e[type=ender_pearl] ~~~ tellraw @a {"rawtext":[{"translate":"chat.ender_pearl_not_allowed"}]}
