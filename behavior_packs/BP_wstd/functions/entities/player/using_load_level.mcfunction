...
execute unless score level data matches 51..60 run clear @s wstd:load_level
...
execute if score level data matches 51..60 unless score diyLevels.isEditMode data matches 1 run clear @s wstd:load_level

# --- 使用情况在预期内时 ---

## 执行到自定义关卡的load_level上
execute if score level data matches 51..60 if score diyLevels.isEditMode data matches 1 run function levels/diy_levels/functions/load_level
