...
execute if score progress.timeLimited record matches 0 run scoreboard players set progress.timeLimited record 1

## 通用函数
function lib/modify_data/level/complete

## 自定义死亡消息
execute as @a run tellraw @s {"rawtext":[{"translate":"chat.deathmessage.time_limited_1","with":{"rawtext":[{"selector":"@s"}]}}]}
