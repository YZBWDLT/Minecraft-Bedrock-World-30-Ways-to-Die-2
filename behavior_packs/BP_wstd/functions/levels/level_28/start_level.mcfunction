...
# 设置level（关卡ID）
scoreboard players set @e[name=level] backend 28

# 调用所有关卡通用函数
function levels/all_levels/start_level

# 标题&音效
titleraw @a title {"rawtext":[{"translate":"title.level_28"}]}
titleraw @a subtitle {"rawtext":[{"translate":"subtitle.level_28"}]}

# 复制房间
structure load level_28 -4 20 104

# 传送玩家
tp @a 4 22 112 facing -3 22 105
