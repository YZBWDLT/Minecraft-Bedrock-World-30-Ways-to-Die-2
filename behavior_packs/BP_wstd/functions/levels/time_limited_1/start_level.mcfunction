...
execute @e[name=language,scores={settings=0}] ~~~ summon wstd:text_display "§c§l合成一个跳过§r§b以通过这关，祝你好运:)" -50 20.9 -78
execute @e[name=language,scores={settings=0}] ~~~ summon wstd:text_display "§c提示：使用下一步以重置所有资源" -50 20.6 -78

execute @e[name=language,scores={settings=1}] ~~~ summon wstd:text_display "§c§lCraft a Skip§r§b by using observer, crossbow," -50 21.2 -78
