...
## 自定义死亡消息
## (设置本关的自定义死亡消息，举例：execute as @a run tellraw @s {"rawtext":[{"translate":"chat.deathmessage.time_limited_1","with":{"rawtext":[{"selector":"@s"}]}}]}
...
## (设置关卡进度为当前进度，举例：execute if score progress.timeLimited record matches 1 run scoreboard players set progress.timeLimited record 2
