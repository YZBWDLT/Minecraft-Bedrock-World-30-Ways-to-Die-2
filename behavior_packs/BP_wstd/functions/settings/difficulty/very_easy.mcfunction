...
## 设置提示、攻略与跳过启用状态
execute if score speedrunMode settings matches 0 run scoreboard players set hintEnabled settings 1
execute if score speedrunMode settings matches 0 run scoreboard players set strategyEnabled settings 1
...
## 提示
execute if score speedrunMode settings matches 0 as @a run tellraw @s {"rawtext":[{"translate":"chat.settings.difficulty.very_easy"}]}
